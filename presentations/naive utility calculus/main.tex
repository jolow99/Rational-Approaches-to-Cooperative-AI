...
\begin{itemize}
    ...
    \item<4-> \textbf{Transition probabilities} ($P_a$): 1 (Prob. of moving to state with given action, not impt in this context) 
    \item<5-> \textbf{Rewards} ($R_a$): Only at goals
    \item<6-> \textbf{In NUC context}: Each goal has its own MDP
\end{itemize}
\end{column}
\end{columns}

\vspace{0.3cm}
\only<7->{\textbf{Purpose:} Compute optimal policies to reach goals efficiently, which observers use to infer agent preferences from observed behavior.}
\end{frame}
...
\vspace{0.1cm}
{\tiny
\textbf{Optimal Policy $\pi^*$:} Maps each state to best action ("What should I do here?")\\
\textbf{Value Function $V^*$:} Expected reward from state with optimal policy ("How good is this state?")\\
...
\textbf{Key Findings:}
\begin{itemize}
    \item \textbf{NUC superiority:} Full NUC (r = 0.86-0.92) significantly outperformed single-event models
    \item \textbf{Information integration:} People combine multiple observations for more precise inferences
    \item \textbf{Quantitative reasoning:} Humans make fine-grained utility calculations, not just simple heuristics
\end{itemize}
\end{column}
\end{columns}
...
\textbf{Implications:}
\begin{itemize}
    \item Naive Utility Calculus provides a unified framework for understanding social cognition
    \item People intuitively use sophisticated cost-benefit analysis to interpret behavior
    \item Multiple observations allow for joint inference of both costs and rewards simultaneously
\end{itemize}
\end{frame}
